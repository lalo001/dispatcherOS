...
//  OS Dispatcher
//
//  Created by Lalo on 10/22/16.
//  Copyright © 2016 Eduardo Valencia. All rights reserved.
//

import UIKit

class SessionOptionsViewController: UIViewController, UITextFieldDelegate {
    ...
    override func loadView() {
        ...
        session = Session(processors: 0, quantumSize: 0, changeTime: 0, blockedTime: 0)
      
        // Create imageView
        imageView = UIImageView(image: UIImage(named: "Processor"))
        imageView.contentMode = .scaleAspectFill
        imageView.translatesAutoresizingMaskIntoConstraints = false
        self.view.addSubview(imageView)
        
        // Add imageView Constraints
        let imageViewHorizontalConstraints = NSLayoutConstraint.constraints(withVisualFormat: "H:[imageView]-(-100)-|", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: ["imageView" : imageView])
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        scrollView.activeTextField = textField
    ...
    func textDidChange(_ sender: UITextField) {
        ...
        if sender.text == "1" && session.status == .processor {
            label.text = "Processor"
        } else if sender.text != "1" && session.status == .processor {
            label.text = "Processors"
        }
    ...
    func continuePressed(_ sender: UIButton) {
        if textField.text == "" || textField.text == "0" {
            ...
            session.processors = Int(textField.text!)!
            textField.resignFirstResponder()
            self.performSegue(withIdentifier: "quantum", sender: self)
        }
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let vc = segue.destination as! QuantumViewController
        vc.session = session
    }
