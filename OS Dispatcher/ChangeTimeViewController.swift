...
class ChangeTimeViewController: UIViewController, UITextFieldDelegate {
    ...
    override func loadView() {
        ...
        // Create imageView
        imageView = UIImageView(image: UIImage(named: "Change"))
    ...
    func continuePressed(_ sender: UIButton) {
        if textField.text == "" {
            session.changeTime = 0
            textField.resignFirstResponder()
            self.performSegue(withIdentifier: "blocked", sender: self)
        } else {
            session.changeTime = Int(textField.text!)!
            textField.resignFirstResponder()
            self.performSegue(withIdentifier: "blocked", sender: self)
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let vc = segue.destination as! BlockedViewController
        vc.session = session
    }
