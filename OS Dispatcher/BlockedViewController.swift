...
class BlockedViewController: UIViewController, UITextFieldDelegate {
    ...
    override func loadView() {
        ...
        // Create imageView
        imageView = UIImageView(image: UIImage(named: "Blocked"))
    ...
    func continuePressed(_ sender: UIButton) {
        if textField.text == "" {
            session.blockedTime = 0
            textField.resignFirstResponder()
            self.performSegue(withIdentifier: "data", sender: self)
        } else {
            session.blockedTime = Int(textField.text!)!
            textField.resignFirstResponder()
            self.performSegue(withIdentifier: "data", sender: self)
        }
    }
